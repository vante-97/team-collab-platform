...
import { getProjects, getTasks, createTask, moveTask, deleteTask, Project, Task } from "@/lib/api";

const COLUMNS = [
  { key: "todo", label: "待办", icon: "○", color: "border-slate-500/30", bg: "bg-slate-500/5", dot: "bg-slate-400" },
  { key: "in_progress", label: "进行中", icon: "◐", color: "border-blue-500/30", bg: "bg-blue-500/5", dot: "bg-blue-400" },
  { key: "done", label: "已完成", icon: "●", color: "border-emerald-500/30", bg: "bg-emerald-500/5", dot: "bg-emerald-400" },
];
...
export default function TasksPage() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const res = await createTask(selectedProject, { title: newTitle.trim(), description: newDesc.trim(), priority: newPriority });
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          <div>
            ...
            <p className="text-white/30 text-sm mt-1">看板式任务管理</p>
          </div>
        ...
        {showCreate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowCreate(false)}>
            <div className="glass-card p-6 w-full max-w-md mx-4 border-white/[0.12] animate-scale-in" onClick={(e) => e.stopPropagation()}>
              ...
              />
              <div className="mb-5">
                <label className="block text-white/50 text-sm mb-2">优先级</label>
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-5 pt-5 pb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-white/60 text-sm">{col.icon}</span>
                      <h3 className="text-white/80 font-medium text-sm">{col.label}</h3>
                      <span className="text-white/20 text-xs ml-1">{colTasks.length}</span>
                    </div>
                  </div>
                  ...
                  <div className="px-3 pb-3 space-y-2 min-h-[200px]">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <h4 className="text-white/90 text-sm font-medium leading-snug flex-1">{task.title}</h4>
                            <span className={`shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium ${pri.color}`}>
                              {pri.label}
                            </span>
                          </div>
                          {task.description && (
                            <p className="text-white/25 text-xs mb-2 line-clamp-2">{task.description}</p>
                          )}
                    ...
                    {colTasks.length === 0 && (
                      <div className="text-center py-10 text-white/10 text-sm">
                        拖拽任务到此处
                      </div>
                    )}
                  </div>
